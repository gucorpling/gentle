...
def flag_dep_warnings(id, tok, pos, lemma, func, parent, parent_lemma, parent_id, children, child_funcs, s_type,
	...
	if pos not in ["CC","SYM"] and func in ["cc","cc:preconj"]:
	...
	if pos in ["LS","SYM"] and lemma != tok and "goeswith" not in child_funcs:
		print("WARN: pos " + pos + " should have lemma identical to token" + inname)
	...
	if pos in ["VBG","VVG","VHG"] and "det" in child_funcs:
		...
		if tok != "prioritizing" and tok != "following" and tok !="coming":
			print(str(id) + docname)
